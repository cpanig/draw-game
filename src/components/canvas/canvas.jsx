...
function useCanvas() {
  ...
  const [isPress, setIsPress] = useState(false); //是否下笔（点击）
...
const Canvas = ({ isDrawer, current: currentDrawer }) => {
  ...
  const { ctx, canvas, current, setCurrent } = useCanvas();
  ...
  const setDraw = (pos) => {
    switch (current) {
      case 1:
        ...
        break;
      case 2:
        return ctx.clearRect(pos.x, pos.y, 100, 100);
      default:
        break;
    }
